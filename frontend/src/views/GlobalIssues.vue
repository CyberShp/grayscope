  ...
  <div class="gs-issues-layout">
    ...
    <aside class="gs-facets">
      ...
      <div class="gs-facet-group">
        <div class="gs-facet-title">项目</div>
        <div
          v-for="p in projectFacets"
          :key="p.value"
          class="gs-facet-item"
          :class="{ active: filters.projectId === p.value }"
          @click="toggleFilter('projectId', p.value)"
        >
          <span>{{ p.label }}</span>
          <span class="gs-facet-count">{{ p.count }}</span>
        </div>
      </div>
      ...
      <div class="gs-facet-group">
        ...
        <div
          v-for="rt in riskTypeFacets.slice(0, showAllRiskTypes ? 99 : 8)"
          :key="rt.value"
          class="gs-facet-item"
        ...
        <div v-if="riskTypeFacets.length > 8" class="gs-facet-more" @click="showAllRiskTypes = !showAllRiskTypes">
          {{ showAllRiskTypes ? '收起' : `展开全部 (${riskTypeFacets.length})` }}
        </div>
      </div>
    </aside>
    ...
    <div class="gs-issues-main">
      ...
      <el-empty v-if="!loading && !filteredFindings.length" description="暂无发现数据" :image-size="60" />
    </div>
    ...
    <aside v-if="selectedFinding" class="gs-issue-detail">
      <div class="gs-issue-detail-header">
        ...
        <span style="font-weight: 600; flex: 1;">{{ selectedFinding.title }}</span>
        <el-button text size="small" @click="selectedFinding = null"><el-icon><Close /></el-icon></el-button>
      </div>
...
const showAllRiskTypes = ref(false)
const filters = ref({ projectId: null, severity: null, module: null, riskType: null })
...
function buildFacets(items, key) {
  const map = {}
  items.forEach(f => { const v = f[key]; if (v) map[v] = (map[v] || 0) + 1 })
  return Object.entries(map).sort((a, b) => b[1] - a[1]).map(([value, count]) => ({ value, count }))
}

const projectFacets = computed(() => {
  const facets = buildFacets(allFindings.value, 'project_id')
  return facets.map(f => {
    const p = appStore.getProjectById(f.value)
    return { ...f, label: p?.name || `项目 ${f.value}` }
  })
})
...
const filteredFindings = computed(() => {
  ...
  if (filters.value.projectId) list = list.filter(f => String(f.project_id) === String(filters.value.projectId))
  ...
  if (sortBy.value === 'severity') list.sort((a, b) => (sevOrder[a.severity] ?? 9) - (sevOrder[b.severity] ?? 9))
  else list.sort((a, b) => (b.risk_score || 0) - (a.risk_score || 0))
  return list
})
...
async function loadFindings() {
  loading.value = true
  try {
    const data = await api.getAllFindings()
...
<style scoped>
.gs-issues-layout { display: flex; height: calc(100vh - var(--gs-nav-height)); }
.gs-issues-main { flex: 1; padding: var(--gs-space-md) var(--gs-space-lg); overflow-y: auto; min-width: 0; }
.gs-issues-toolbar { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--gs-space-md); }
.gs-issues-count { font-size: var(--gs-font-sm); color: var(--gs-text-secondary); font-weight: 600; }
.gs-issues-list { display: flex; flex-direction: column; }
.gs-issue-item { display: flex; align-items: flex-start; gap: var(--gs-space-sm); padding: var(--gs-space-sm) var(--gs-space-md); border-bottom: 1px solid var(--gs-border-light); cursor: pointer; transition: background var(--gs-transition); }
...
.gs-issue-title { font-size: var(--gs-font-base); font-weight: 500; color: var(--gs-text-primary); margin-bottom: 4px; }
.gs-issue-meta { display: flex; gap: var(--gs-space-md); font-size: var(--gs-font-xs); color: var(--gs-text-muted); }
...
.gs-issue-detail { width: 380px; flex-shrink: 0; border-left: 1px solid var(--gs-border); background: var(--gs-surface); overflow-y: auto; }
.gs-issue-detail-header { display: flex; align-items: center; gap: var(--gs-space-sm); padding: var(--gs-space-md); border-bottom: 1px solid var(--gs-border-light); }
.gs-issue-detail-body { padding: var(--gs-space-md); }
.gs-detail-section { margin-bottom: var(--gs-space-lg); }
.gs-detail-label { font-size: var(--gs-font-xs); font-weight: 600; color: var(--gs-text-secondary); text-transform: uppercase; letter-spacing: 0.3px; margin-bottom: var(--gs-space-xs); }
.gs-detail-value { font-size: var(--gs-font-base); color: var(--gs-text-primary); line-height: 1.5; }
.gs-detail-value code { background: #F5F5F5; padding: 2px 4px; border-radius: 3px; font-family: var(--gs-font-mono); font-size: var(--gs-font-sm); }
.gs-detail-tags { display: flex; flex-wrap: wrap; gap: var(--gs-space-xs); }
.gs-tag { padding: 2px 8px; background: #F0F0F0; border-radius: 3px; font-size: var(--gs-font-xs); }
.gs-code-block { background: #1E1E1E; color: #D4D4D4; padding: var(--gs-space-md); border-radius: var(--gs-radius-md); font-family: var(--gs-font-mono); font-size: var(--gs-font-sm); overflow-x: auto; white-space: pre-wrap; word-break: break-all; max-height: 300px; overflow-y: auto; }
.gs-facet-more { padding: 4px 8px; font-size: var(--gs-font-xs); color: var(--gs-text-link); cursor: pointer; }
.gs-test-suggestion-section .gs-detail-label { display: flex; align-items: center; color: var(--gs-primary); }
.gs-test-suggestion-card { background: rgba(75,159,213,0.04); border: 1px solid rgba(75,159,213,0.15); border-radius: var(--gs-radius-md); padding: var(--gs-space-md); font-size: var(--gs-font-sm); color: var(--gs-text-primary); line-height: 1.6; }
.gs-ts-objective { margin-bottom: 8px; }
.gs-ts-steps ol { margin: 4px 0 8px 0; padding-left: 18px; color: var(--gs-text-secondary); }
.gs-ts-steps ol li { padding: 2px 0; }
.gs-ts-steps ol li::marker { color: var(--gs-primary); font-weight: 600; }
.gs-ts-expected { color: var(--gs-success); font-weight: 500; }
</style>
